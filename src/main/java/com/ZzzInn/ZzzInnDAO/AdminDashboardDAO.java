...
public class AdminDashboardDAO {
    ...
    public AdminDashboardDAO() throws Exception {
    ...
    public int getTotalGuests() throws SQLException {
        String query = "SELECT COUNT(DISTINCT CustomerID) FROM customer_room";
        try (PreparedStatement ps = conn.prepareStatement(query);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) return rs.getInt(1);
        }
    ...
    public int getRegisteredUsers() throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(query);
            ...
            while (rs.next()) {
                ...
                activities.add(new String[] {
                    "Guest \"" + guest + "\" " + activity + " (Room " + room + ")", time
                });
            }
